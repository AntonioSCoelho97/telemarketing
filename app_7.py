...
def main():
    ...
    if (data_file_1 is not None):
        ...
        # PLOTS
        bank_raw_target_perc_dict = bank_raw_target_perc.to_dict()
        bank_dict = bank_raw_target_perc_dict.values()        
        bank_target_perc_dict = bank_target_perc.to_dict()
        bank_dict_2 = bank_target_perc_dict.values()
        cores = ['C0', 'C1']
        if graph_type == 'Barras':
            ax[0].bar(pd.DataFrame(bank_dict).columns, pd.DataFrame(bank_dict).iloc[0], color = cores)
            ax[0].bar_label(ax[0].containers[0])
            ax[0].set_title('Dados brutos',
                            fontweight ="bold")
            
            ax[1].bar(pd.DataFrame(bank_dict_2).columns, pd.DataFrame(bank_dict_2).iloc[0], color = cores)
            ax[1].bar_label(ax[1].containers[0])
            ax[1].set_title('Dados filtrados',
                            fontweight ="bold")
        else:
            ax[0].pie(pd.DataFrame(bank_dict).iloc[0], labels=pd.DataFrame(bank_dict).columns, autopct='%.2f')
            ax[0].set_title('Dados brutos',
                            fontweight ="bold")

            ax[1].pie(pd.DataFrame(bank_dict_2).iloc[0], labels=pd.DataFrame(bank_dict_2).columns, autopct='%.2f')
            ax[1].set_title('Dados filtrados',
                            fontweight ="bold")
